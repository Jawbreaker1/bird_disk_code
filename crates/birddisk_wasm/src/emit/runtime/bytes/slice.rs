...
pub(super) fn emit_bytes_slice(emitter: &mut WatEmitter, max_bytes_len: i32) {
    ...
    emitter.push_line("(local $out i32)");
    ...
    emitter.push_line("if");
    emitter.indent();
    emitter.push_line(format!("i32.const {TRAP_ARRAY_OOB}"));
    emitter.push_line("call $bd_trap");
    emitter.dedent();
    emitter.push_line("end");
    ...
    emitter.push_line("local.set $arr_len");
    emitter.push_line("local.get $arr_len");
    emitter.push_line("i64.extend_i32_u");
    emitter.push_line("local.set $arr_len64");
    ...
    emitter.push_line("local.get $arr_len64");
    ...
    emitter.push_line("local.set $out");
