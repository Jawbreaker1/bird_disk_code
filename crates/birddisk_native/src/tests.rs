...
#[test]
fn native_std_json_escape_decoding_module() {
    ...
    let result = run_module_source(source, "json_escape_decode_native");
    assert_eq!(result, 1);
}

#[test]
fn native_std_json_decode_invalid_escape_errors() {
    ...
    let err = run_module_source_error(source, "json_invalid_escape_native");
    assert_eq!(err.code, Some("E0400"));
}

#[test]
fn native_std_json_encode_rejects_unsupported_control_chars() {
    let source = "import std::json.\nimport std::string.\n\nrule main() -> i64:\n  set data: u8[] = [1].\n  set value: string = std::string::from_bytes(data).\n  set encoded: string = std::json::encode_string(value).\n  yield std::string::len(encoded).\nend\n";
    let err = run_module_source_error(source, "json_control_char_native");
    assert_eq!(err.code, Some("E0400"));
}
...
#[test]
fn native_std_webconfig_helpers() {
    ...
    let result = run_module_source(source, "webconfig_helpers_native");
    assert_eq!(result, 1);
}

#[test]
fn native_std_webfiles_helpers() {
    ...
    let result = run_module_source(source, "webfiles_helpers_native");
    assert_eq!(result, 1);
}
